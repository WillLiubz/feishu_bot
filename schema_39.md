...
| 群体 | 定义 |
|---|---|
| 付费玩家 | 分析窗口内有过充值记录（`raw_scribe_log.pay`） |
| 沉默玩家 | 分析窗口前 30 天内曾充值，但窗口内未充值 |
| 免费玩家 | 分析窗口内活跃，且分析窗口 + 沉默窗口内均无充值记录 |

- 分析窗口默认：近 7 天（用户可在提问时覆盖）。
- 沉默窗口默认：30 天。
- Top 明细默认：每群 100 人。

> 注：SQL 内部使用英文列别名（如 `pay_diamonds`、`user_count`），输出 Excel 通过 `columns` 映射显示中文表头。
