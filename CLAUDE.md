...
- `config_db.database` 为 **GM 运营库**（`activity` / `server` / `operator` / `game_resource` 等）；`config_db.static_database` 为 **游戏静态库**（`static_item` / `static_language` / `static_npc_base` 等道具/英雄/技能中文名来源）。两个库连接参数相同，仅库名不同；未填写 `static_database` 时默认与 `database` 相同。
- 查道具 ID 的中文名时，需要把 `query_config` 的 `database` 参数设为 `static_database`，然后查 `static_item` 表：
  - 女 1（39）：`SELECT id, name, describe FROM static_item WHERE id = ?`
  - 女 2 页游（160）/ 女 3（312）：`name` 字段可能是语言 key，需要 `LEFT JOIN static_language b ON a.name = b.id` 取 `b.text` 作为中文名（实际以库中数据为准）。
