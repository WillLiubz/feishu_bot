...
- `config_db.database` 为 **GM 运营库**（`activity` / `server` / `operator` / `game_resource` 等）；`config_db.static_database` 为 **游戏静态库**（`static_item` / `static_language` 等道具中文名来源）。两个库连接参数相同，仅库名不同；未填写 `static_database` 时默认与 `database` 相同。
- 查道具 ID 的中文名时，把 `query_config` 的 `database` 参数设为 `static_database`，然后按游戏查对应表：
  - 女 1（39）：`SELECT id, name, describe FROM static_item WHERE id = ?`
  - 女 2 页游（160）/ 女 3（312）：`SELECT ident, name, description FROM game_item WHERE game_id = <game_id> AND ident = ?`
  - 女 2 页游（160）/ 女 3（312）通用资源/货币：`SELECT id_name, name, description FROM game_resource WHERE game_id = <game_id> AND id_name = ?`
- 160/312 的 `game_item` 表需按 `game_id` 过滤（同一个 GM 库服务多个游戏）；`ident` 是道具/装备 ID，`name` 为中文名，`description` 为描述。
- 39 的 `static_item` 中 `id` 为道具 ID，`name` 为中文名，`describe` 为描述。
...
>
> **道具中文名来源（已连库确认）**：静态库 `static_database` 中的 `static_item` 表。常用字段：`id`（道具 ID）、`name`（中文名）、`describe`（描述）、`type`（类型）、`quality`（品质）。示例：
> ```sql
...
>
> **道具中文名来源（已连库确认）**：GM 运营库 `xgame_gm` 中的 `game_item` 表（同一库服务多个游戏，**必须带 `game_id = 160` 过滤**）。常用字段：`ident`（道具/装备 ID）、`name`（中文名）、`description`（描述）。示例：
> ```sql
> SELECT ident, name, description
> FROM game_item
> WHERE game_id = 160 AND ident = '601229';
> ```
> 通用资源/货币名称查 `game_resource` 表，同样需 `game_id = 160`：
> ```sql
> SELECT id_name, name, description
> FROM game_resource
> WHERE game_id = 160 AND id_name = '6';
> ```
...
> **说明**：实际字段需以配置库 `DESC <表名>` 结果为准；不确定表名时先用 `SHOW TABLES` 探索。游戏 312 数仓各表（`gamelog_raw` 与 `gameeco_raw`，含日志表与快照表）的 `role_id` 均为 **VARCHAR 字符串**（已 typeof 实测确认）：过滤时必须加引号写 `role_id = '123456'` / `IN ('id1','id2',...)`，不要写不加引号的整数比较（会导致全表扫描超时），也不要用 `CAST(role_id AS VARCHAR)` 包装列（阻断谓词下推）。`Server` 表中的 `dc_server_id` 通常与数仓中的 `server_id` 对应。
>
> **直购记录（2026-07 源码+实测确认）**：`gamelog_raw.v_presto_log_payrecharge` 的 `pay_type` 恒为 `'1'`（源码 `game_log.go` 硬编码），文档标注的 "2=购买商品" 不生效。区分普通充值与直购看 `pay_itemid`：普通充值 = `'0'`，直购 = `'activityId:giftId'`（activityId 见源码 `const.pb.go` `DIRECT_PURCHASE_ACT_ID`，14=新手直购、13=新月卡、9=天使通行证、7/8=商店等）。查直购用 `strpos(pay_itemid, ':') > 0`，商品 ID 用 `split_part(pay_itemid, ':', 2)`。
>
> **道具中文名来源（已连库确认）**：GM 运营库 `xgame_gm` 中的 `game_item` 表（同一库服务多个游戏，**必须带 `game_id = 312` 过滤**）。常用字段：`ident`（道具/装备 ID）、`name`（中文名）、`description`（描述）。示例：
> ```sql
> SELECT ident, name, description
> FROM game_item
> WHERE game_id = 312 AND ident = '2014003';
> ```
> 通用资源/货币名称查 `game_resource` 表，同样需 `game_id = 312`：
> ```sql
> SELECT id_name, name, description
> FROM game_resource
...
>
> **直购记录（2026-07 源码+实测确认）**：`gamelog_raw.v_presto_log_payrecharge` 的 `pay_type` 恒为 1（源码 `vip.go` 两处 `PayRechargeLog` 均传 `PAY_RECHARGE_BUY_DIAMOND`），文档标注的 "2=购买商品" 不生效。区分看 `pay_itemid`：普通充值 = `'0'`，直购 = `'actId:giftId'`。⚠ 源码 `fmt.Sprintf` 参数类型错误，数仓中实为 `'%!d(string=3:8001)'` 形态：取 actId 用 `replace(split_part(pay_itemid,':',1), '%!d(string=', '')`，取 giftId 用 `replace(split_part(pay_itemid,':',2), ')', '')`。actId 枚举见 `game.pb.go` `DIRECT_TYPE`（1=活动直购、2=商店、3=新月卡、4=周年新通行证、6=基金新通行证、7=刮刮卡、8=代金券、9=自定义礼包、10=王之财宝、11=渔场通行证）。查直购用 `strpos(pay_itemid, ':') > 0`。
...
> **字段约定**：`raw_scribe_log.*` 表中的 `iuid` 是玩家内部唯一 ID，`ouid` 是平台账号/通行证；两者均为字符串，按玩家过滤时必须加引号（`iuid = '123456'`），不要写不加引号的整数比较。业务字段在 `custom_pra1` ~ `custom_pra8` 中，具体含义见 `schema_39.md`。**过滤游戏必须使用字符串 `gameid = '39'`，不要写 `game_id = 39`，后者会导致全表扫描超时。** 数值列（如 `custom_pra3`）在 Presto 中是 VARCHAR，求和/排序时必须 `CAST(... AS BIGINT)` 或 `CAST(... AS DOUBLE)`。
>
